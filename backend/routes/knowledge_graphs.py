...
async def scrape(request: Request):
    ...
    if title:
        try:
            ...
            verify_response = await verify_news(Request(scope={"type": "http"}, receive=None, send=None), payload)
            return verify_response
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to scrape and verify: {e}")
    else:
